...
- **Robust ESPHome deploy script** (`extra/deploy-esphome-to-ha.ps1`, [#97](https://github.com/genestealer/everblu-meters-esp8266-improved/pull/97)): a runnable replacement for the ad-hoc `Remove-Item`/`Copy-Item` deploy notes that clears read-only attributes and uses `robocopy /MIR`, so a partial/aborted copy can no longer leave a broken component on the Home Assistant share.
