...
- **Log format strings are now held in flash rather than DRAM on ESP8266.** `TS_PRINTF`, `TS_PRINTLN` and the `LOG_D`/`LOG_I`/`LOG_W`/`LOG_E`/`LOG_V` macros route through `printf_P(PSTR(...))` and `println(F(...))`, and the remaining bare `Serial.print*` banner lines in `main.cpp` use the same helpers. A string literal otherwise lands in `.rodata`, which on ESP8266 is DRAM, not flash. Static RAM drops a further 10.2 percentage points. Output is unchanged. Note for contributors: on ESP8266 these macros now require a compile-time literal format string, so `TS_PRINTF(someCharPointer)` will not compile; use `TS_PRINTF("%s\n", someCharPointer)`. ESP32 and the native host build keep the plain form.
- **The WiFi serial monitor is now compiled out entirely when `WIFI_SERIAL_MONITOR_ENABLED` is `0`** (the default). The flag previously only skipped the calls to `wifiSerialBegin()`/`wifiSerialLoop()`, so the 8 KB transmit ring buffer, the 1 KB `printf` scratch buffer and the `WiFiServer`/`WiFiClient` objects were linked in regardless. Setting the flag to `1` restores the previous behaviour and its memory cost. When disabled, `Serial` is no longer remapped through the mirror and resolves to the hardware UART; `WiFiSerial` is aliased to `Serial` so existing call sites are unaffected. ESPHome builds, which never enabled the monitor, also drop the unused buffers.
- **Combined effect of the two items above on ESP8266 static RAM: 82.5% to 58.6% of the 81920-byte budget** (67604 to 48016 bytes, a 19588-byte saving). With the WiFi serial monitor enabled the figure is 69.5%, down from 82.5%.
