...
- **LQI reported with the CRC_OK bit still set** (both builds, shared driver `src/core/cc1101.cpp`). The CC1101 `LQIVAL` register packs the frame `CRC_OK` flag into bit 7 and the actual link-quality estimate into bits 6:0. `get_meter_data_for_meter()` stored the raw register directly, so a good frame reported LQI values ≥ 128 (e.g. `0xF3` = 243 instead of the true `115`), and the value published to Home Assistant was inflated by 128 whenever CRC passed. The stored LQI now masks bit 7 (`& 0x7F`), and `calculateLQIToPercentage()` (`src/core/utils.cpp`) now scales over the correct 7-bit range (`0–127`) instead of `0–255`, so the LQI percentage is no longer capped at ~50%. The `[CC1101] ... lqi=` debug lines are also masked for consistency.
