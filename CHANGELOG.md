...
- **The WiFi serial monitor is now compiled out entirely when `WIFI_SERIAL_MONITOR_ENABLED` is `0`** (the default). The flag previously only skipped the calls to `wifiSerialBegin()`/`wifiSerialLoop()`, so the 8 KB transmit ring buffer, the 1 KB `printf` scratch buffer and the `WiFiServer`/`WiFiClient` objects were linked in regardless. ESP8266 static RAM drops from 82.5% to 71.1% of the 81920-byte budget (67604 to 58236 bytes, a 9368-byte saving). Setting the flag to `1` restores the previous behaviour and its memory cost. When disabled, `Serial` is no longer remapped through the mirror and resolves to the hardware UART; `WiFiSerial` is aliased to `Serial` so existing call sites are unaffected. ESPHome builds, which never enabled the monitor, also drop the unused buffers.
