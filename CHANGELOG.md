...
- **ESPHome build broke on shared files using `TS_PRINTLN` / `TS_PRINTF`.** The timestamped-Serial log macros were defined only in the MQTT (`#else`) branch of `src/core/logging.h`, but shared service files (`frequency_manager.cpp`, `schedule_manager.cpp`) use them — so the ESPHome component failed to compile with `'TS_PRINTLN' was not declared in this scope`. The ESPHome branch of `logging.h` now also defines `TS_PRINTLN` / `TS_PRINTF`, routing them through the ESPHome logger (`ESP_LOGI`) so shared code compiles and logs in both builds.
