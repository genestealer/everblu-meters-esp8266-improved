...
- **A frequency scan no longer gives up because the frequency that first answered has gone quiet.** When a run of misses triggers the "meter is awake" check, the scan re-read the frequency where acquisition first got a response. The response band drifts while a sweep runs, so on a long scan that frequency can be silent minutes later while the meter is still answering happily elsewhere: one reported scan discarded 45 live decodes spanning 177 kHz because its check point, 176 kHz away and five minutes stale, did not reply. The check now uses the most recent frequency that decoded, in whichever stage it decoded. Both builds ([#104](https://github.com/genestealer/everblu-meters-esp8266-improved/issues/104)).
