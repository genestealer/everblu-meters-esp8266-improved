...
- **`tuned_frequency` and `frequency_estimate` MQTT sensors stayed blank/unknown in Home Assistant** (standalone build). The firmware published the Home Assistant discovery config for both sensors but never published their state topics, so they never received a value. `adaptiveFrequencyTracking()` now publishes `tuned_frequency` (MHz) and `frequency_estimate` (kHz, from the CC1101 `FREQEST` register) after every read, and the boot-time publish block now seeds `tuned_frequency` so the calibrated value appears immediately on connect.

### Removed

- **Unused `MQTTDataPublisher` class** (`src/adapters/implementations/mqtt_data_publisher.{cpp,h}`). The standalone (MQTT) build publishes to MQTT directly from `src/main.cpp` and never instantiated this adapter; only the ESPHome build uses the `IDataPublisher` abstraction (via `ESPHomeDataPublisher`). The dead class has been removed to avoid the misleading impression that it is live in the standalone path.
