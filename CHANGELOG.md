...
- **Codecov code coverage reporting**: a dedicated `coverage.yml` GitHub Actions workflow runs the native PlatformIO test suite with gcov instrumentation (`--coverage`) and uploads results to Codecov via `gcovr`. Coverage is tracked for `crc_kermit.cpp` and `radian_parser.cpp` — the two files exercised by the native fixture tests. A Codecov badge has been added to the README.
...
- **LQI reported with the CRC_OK bit still set, and its percentage inverted** (both builds, shared driver `src/core/cc1101.cpp` / `src/core/utils.cpp`). The CC1101 `LQIVAL` register packs the frame `CRC_OK` flag into bit 7 and the actual link-quality estimate into bits 6:0. `get_meter_data_for_meter()` stored the raw register directly, so a good frame reported LQI values ≥ 128 (e.g. `0xF3` = 243 instead of the true `115`), and the value published to Home Assistant was inflated by 128 whenever CRC passed. The stored LQI now masks bit 7 (`& 0x7F`). In addition, the CC1101 LQI is an accumulated *demodulation-error* metric where a **lower value means a better link** — the previous `calculateLQIToPercentage()` scaled it as "higher is better" over `0–255`, so a strong signal read as a low quality percentage. It now scales over the correct 7-bit range and **inverts** the mapping (LQI `0` → `100%`, LQI `127` → `0%`), so the LQI percentage genuinely tracks link quality. The ESPHome build published its `lqi_percentage` sensor through a separate, divergent `ESPHomeDataPublisher::calculateLqiPercentage()` that still used the old `0–255` higher-is-better formula; it has been aligned with the shared logic so both builds now report identical values. The `[CC1101] ... lqi=` debug lines are also masked for consistency.
- **Missing newlines in two CC1101 debug log lines** (`src/core/cc1101.cpp`) caused adjacent log messages to run together (e.g. `...sts:0x07[16:24:49][METER] Waiting for ACK...` and `...F_est=255[16:24:49][CC1101] GDO0 triggered...`). The `rssi/lqi/F_est` and `tmo/free_byte/sts` diagnostic lines now terminate with `\n`.
- **`F_est` printed as unsigned in CC1101 debug output** (`src/core/cc1101.cpp`). The CC1101 `FREQEST` register is a signed `int8_t`, so a small negative offset was logged as e.g. `F_est=255` instead of `-1`. The debug lines now print it signed (`%d` with an `int8_t` cast), matching the correctly-signed `FREQEST:` line.
