...
- **CC1101 RX bandwidth widened 58 kHz → 270 kHz and FOC_LIMIT raised to ±BW/4** (`MDMCFG4 0xF6 → 0x66`, `FOCCFG 0x1D → 0x1E`) in the shared radio driver (both MQTT and ESPHome builds). The narrow 58 kHz filter with ±7.25 kHz frequency-offset compensation could not tolerate a CC1101 reference-crystal error larger than a few ppm, which is why software frequency scanning was needed for basic operation. The 270 kHz filter gives the chip **±67.7 kHz of automatic carrier-offset correction (~±156 ppm at 433 MHz)**, so the radio locks onto the meter at the nominal 433.82 MHz even with a badly off-spec crystal — usually with no scan at all. The RADIAN signal is only ~15 kHz wide, so the extra bandwidth costs ~6.7 dB of noise floor, negligible against the typical >20 dB link margin. Frequency scanning remains available as a fallback for weak signals or extreme drift.
- **Removed the Fast frequency scan entirely** (MQTT and ESPHome builds). It was redundant: the two-phase Deep scan already does a coarse window-mapping pass followed by a fine zoom, superseding the old coarse-only Fast scan. Removed `performFastFrequencyScan()`, the `fast_scan` MQTT command + Home Assistant button, and the ESPHome `fast_scan_button` config option. Use the Deep scan (`deep_scan_button` / `deep_scan` command) instead.
- **Two-phase deep frequency scan algorithm** (MQTT and ESPHome builds):
  ...
  - **Zoom step clamped to CC1101 hardware minimum**: the CC1101 FREQ register resolution is `Fxosc / 2^16 = 26 MHz / 65536 ≈ 397 Hz`. Steps finer than this silently round to the same register value, retesting the same physical frequency. The zoom step is now `max(scanStep × 0.25, 397 Hz)`, ensuring every step advances at least one register and produces a unique tuning point. For the narrow 1 kHz auto-scan this raises the zoom step from 250 Hz (sub-resolution) to 397 Hz (one register step).
