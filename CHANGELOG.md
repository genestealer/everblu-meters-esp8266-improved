...
- **`Diagnostic Report` MQTT button**: the standalone MQTT build exposes the same report as a Home Assistant button and on the `<base>/diagnostic_report` topic (payload `report`). The report body was moved into the shared CC1101 driver, so both integrations print byte-for-byte comparable blocks and a bug report needs only one set of instructions. The report goes to the serial / WiFi serial log rather than to MQTT, since it is a diagnostic dump rather than an entity state.
