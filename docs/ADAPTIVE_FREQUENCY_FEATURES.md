...
- FOC_PRE_K = 3K (mediu loop gain before sync word)
...
// If no valid frequency offse found and auto-scan is enabled, perform wide initial scan
...
2. Note FREQEST values andcumulative error
...
2. Wait for temperatur stabilization (~30 min)
...
- Verify meter is in wke window (check time_start/time_end in previous reads)
