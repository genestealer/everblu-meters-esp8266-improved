...
Scans run in stages on both targets: wide acquisition in nominal 10 kHz jumps, one nominal 2.5 kHz fallback pass if that finds nothing, then a short refinement window around the first response. Candidates are ranked on decode reliability before FREQEST is consulted, and the winner is verified before it is saved. A local recovery scan starts around the saved tuning and widens only if nothing answers.

Two things make a scan cost less than it used to:

- **Cost after the first response is fixed.** The scanner used to walk outwards from the first decode until five consecutive misses closed each edge, then sweep everything in between at 793 Hz. That treats a missed reply as evidence the tuning is wrong. It is not: the meter decodes over a span far wider than the scan can resolve, and a miss usually means the meter was not transmitting just then. One field log has the same setting reading nothing at 18:31:55 and reading the meter at full strength ninety seconds later. The scan mapped a 171 kHz band out of that, queued a 25 minute sweep, and drew its upper edge below the frequency the meter was known to answer on. It now samples nine settings spanning ±9.5 kHz around the first response, twice each, and goes to verification: about 20 reads rather than 400. A response that cannot be reproduced no longer ends the scan either; the sweep carries on from where it left off, up to twice per scan.
