...
- **It stands down when the meter stops answering.** Meters transmit on a duty cycle, and a long sweep is itself enough to quieten one. Every stage used to read "no reply" as "wrong frequency", so a meter that went to sleep part way through was recorded as a miss at every remaining step. One reported scan spent 25 minutes re-testing frequencies that had answered minutes earlier, then failed and restored the old tuning anyway. The scan now re-reads the frequency that first responded once misses build up, and stops with `Meter stopped answering - scan stopped, try again later` if that is silent too. The stored calibration is untouched, so retrying later costs nothing.
