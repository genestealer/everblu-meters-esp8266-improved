...
#include "net_logger.h"     // Network logging helper (mirrors Serial to TCP)
#if defined(ESP8266)
...
void onUpdateData()
{
  ...
  logPrintln("Updating data from meter...");
  logPrintf("Retry count : %d\n", _retry);
  logPrintf("Reading schedule : %s\n", readingSchedule);
  ...
  logPrintf("Current date (UTC) : %04d/%02d/%02d %02d:%02d/%02d - %ld\n", ptm->tm_year + 1900, ptm->tm_mon + 1, ptm->tm_mday, ptm->tm_hour, ptm->tm_min, ptm->tm_sec, (long)tnow);
  ...
  if (meter_data.reads_counter == 0 || meter_data.liters == 0)
  {
    ...
    logPrintf("Unable to retrieve data from meter (attempt %d/%d)\n", _retry + 1, MAX_RETRIES);
    ...
    if (_retry < MAX_RETRIES - 1)
    {
      ...
      logPrintf("Scheduling retry in 10 seconds... (next attempt %d/%d)\n", _retry + 1, MAX_RETRIES);
    ...
    else
    {
      ...
      logPrintf("Max retries (%d) reached. Entering 1-hour cooldown period.\n", MAX_RETRIES);
      mqtt.publish("everblu/cyble/active_reading", "false", true);
      mqtt.publish("everblu/cyble/cc1101_state", "Idle", true);
  ...
  logPrintln("Data from meter:");
  logPrintf("Liters : %d\nBattery (in months) : %d\nCounter : %d\nRSSI : %d\nTime start : %s\nTime end : %s\n\n", meter_data.liters, meter_data.battery_left, meter_data.reads_counter, meter_data.rssi, timeStartFormatted, timeEndFormatted);
  logPrintf("RSSI (dBm) : %d\n", meter_data.rssi_dbm);
  logPrintf("RSSI (percentage) : %d\n", calculateMeterdBmToPercentage(meter_data.rssi_dbm));
  logPrintf("Signal quality (LQI) : %d\n", meter_data.lqi);
  logPrintf("Signal quality (LQI percentage) : %d\n", calculateLQIToPercentage(meter_data.lqi));
  ...
  if (meter_data.history_available)
  {
    ...
    Serial.println("===================================\n");
    logPrintf("\n=== Historical Data (%d months) ===\n", num_history);
    logPrintln("Month  Volume (L)  Usage (L)");
    logPrintln("-----  ----------  ---------");
    ...
    logPrintf("  Now  %10d  %9u (current month)\n", meter_data.liters, currentMonthUsage);
    logPrintln("===================================\n");
    logPrintf("Publishing JSON attributes (%d bytes): %s\n\n", strlen(historyJson), historyJson);
    logPrintf("> Published %d months historical data (current month usage: %u L)\n",
              num_history, currentMonthUsage);
  }
  ...
  logPrintln("Data update complete.\n");
}
...
void onScheduled()
{
  ...
  if (isReadingDay(ptm) && timeMatch && ptm->tm_sec == 0)
  {
    ...
    if (lastFailedAttempt > 0 && (millis() - lastFailedAttempt) < RETRY_COOLDOWN)
    {
      ...
      logPrintf("Still in cooldown period. %lu seconds remaining.\n", remainingCooldown);
    ...
    logPrintln("It is time to update data from meter :)");
...
void onConnectionEstablished()
{
  ...
  logPrintln("Connected to MQTT Broker :)");

  Serial.println("> Configure time from NTP server. Please wait...");
  logPrintln("> Configure time from NTP server. Please wait...");
  ...
  logPrintf("Current date (UTC) : %04d/%02d/%02d %02d:%02d/%02d - %ld\n", ptm->tm_year + 1900, ptm->tm_mon + 1, ptm->tm_mday, ptm->tm_hour, ptm->tm_min, ptm->tm_sec, (long)tnow);
  logPrintf("Configured UTC offset: %+d minutes\n", offsetMin);
  logPrintf("Current date (UTC+offset): %04d/%02d/%02d %02d:%02d/%02d - %ld\n",
            plocal->tm_year + 1900, plocal->tm_mon + 1, plocal->tm_mday,
            plocal->tm_hour, plocal->tm_min, plocal->tm_sec, (long)tlocal);
  ...
  logPrintln("> Configure Arduino OTA flash.");
  ArduinoOTA.onStart([]()
                     {
    String type;
...
// On platforms with native USB Serial (e.g. some ESP32 cores) wait briefly for host to open the port
  ...
  logPrintln("Everblu Meters ESP8266/ESP32 Starting...");
  logPrintln("Water usage data for Home Assistant");
  logPrintln("https://github.com/genestealer/everblu-meters-esp8266-improved");
  logPrintf("Target meter: 20%02d-%07lu\n\n", METER_YEAR, (unsigned long)METER_SERIAL);
  ...
  if (!validateConfiguration())
  {
    ...
    logPrintln("\n*** FATAL: Configuration validation failed! ***");
    logPrintln("*** Fix the errors in private.h and reflash ***");
    logPrintln("*** Device halted - will not continue ***\n");
    while (1)
    {
  ...
  logPrintln("✓ Configuration valid - proceeding with initialization\n");
